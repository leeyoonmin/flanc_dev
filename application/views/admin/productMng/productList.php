<div class="divContents productList">
...
<div class="divGrid is_check">
  <table>
      <tr class="head">
        <td class="checkBox">□</td><td>상품코드</td><td>사진</td><td>상품명</td><td>판매가</td><td>등록일자</td>
      </tr>
      <?php
        foreach($GRID_DATA as $item){
          echo "
          <tr class=\"body\">
            <td class=\"checkBox\" idxkey=\"".$item->PRD_ID."\">□</td>
            <td>".$item->PRD_ID."</td>
            <td><img src=\"/resource/img/product/".$item->PRD_ID."/".$item->IMG_NAME.".".$item->IMG_EXTENSION."\"></td>
            <td class=\"PRD_NAME_TD\">".$item->PRD_NAME."</td>
            <td>".number_format($item->PRD_SALES_PRICE)."</td>
